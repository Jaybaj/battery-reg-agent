...
export default function ChatMessageBubble({ message, jurisdictionFilter, onOpenSection }: ChatMessageBubbleProps) {
  if (message.role === "user") {
    return (
      <div className="flex justify-end">
        <div className="max-w-[75%] rounded-2xl rounded-br-sm bg-slate-900 px-4 py-2.5 text-[0.925rem] leading-relaxed text-white">
          {message.content}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex justify-start">
      <div className="max-w-[85%] rounded-2xl rounded-bl-sm border border-slate-200 bg-white px-5 py-4 shadow-sm">
